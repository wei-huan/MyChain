...
// NewRouter 建立短连接路由
func NewRouter(to time.Duration) *Router {
	var r Router
	r.to = to
...
func (r *Router) AddRoute(name string, addr interface{}) error {
...
// Delete 删除peer
func (r *Router) Delete(s string) error {
	fmt.Printf("删除节点：%v\n", s)
	r.Lock()
	defer r.Unlock()
	delete(r.Pool, s)
...
// DispatchAll 广播消息
func (r *Router) DispatchAll(msg []byte) map[string][]byte {
	var l sync.Mutex
	peers := r.FetchPeers()
...
// clone
func (r Router) FetchPeers() map[string]interface{} {
...
// Dispatch 单点传输
func (r *Router) Dispatch(name string, msg []byte) ([]byte, error) {
...
// clone
func (r *Router) getPeer(name string) (*EndPointS, error) {
	p2 := &EndPointS{}
	r.RLock()
	defer r.RUnlock()
	if _, ok := r.Pool[name]; !ok {
		return p2, Error(ErrUnknowuPeer)
	}
	p2.Addr = r.Pool[name].Addr
	return p2, nil
}

func (r *Router) dispatch(addr string, msg []byte) ([]byte, error) {
	var resp []byte
...
func (r *Router) read(io io.Reader, to time.Duration) ([]byte, error) {
